 ...
 ***************************************************************************** 
 *
 * Project:      EVT_Mcast_Master
 * Description:  Example program
 *
 * (c) 2019      by Emergent Vision Technologies Inc
 *               www.emergentvisiontec.com
 *****************************************************************************
 *   The EVT_Mcast_Master example opens the camera and configures
 *   it for highest speed and starts multicast streamingg. It counts dropped frames 
 *   and then calculates the effective frame rate if specified to subscribe the streaming.
 *
 *   Command line usage:
 *   EVT_Mcast_Master <camera=0,1,2,...> <PixelFormat> <multicast ip> <multicast port> <subscribe multicast stream> [CPU]
 *   ie.
 *   EVT_Mcast_Master 0 BayerRG8 239.255.255.250 60646 0
 *   This command starts the multicast streaming, but doesn't subscribe it.
 *   
 *   ie.
 *   EVT_Mcast_Master 0 BayerRG8 239.255.255.250 60646 1 2
 *   This command starts the multicast streaming and subscribe it, and grab the the frames with CPU 2.
  ****************************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  bool multicastSubscribe;

  printf("-------------------------------"); printf("\n");
  printf("Mcast_Master : Example program "); printf("\n");
  printf("-------------------------------"); printf("\n");
  ...
  if(argc == 7)
  {
    cpuToUse = atoi(argv[6]); // 1 the first cpu, 2 the second cpu
    if(cpuToUse > 0)
    {
  ...
  if(argc >= 6)
  {
    ...
    multicastSubscribe = atoi(argv[5]);
    printf("Multicast Ip: %s, port: %d.\n", multicastIp, multicastPort);
  }
  else
  {
...
//  EVT_CameraSetEnumParam(&camera,      "PixelFormat", "YUV444Packed");
  ...
  camera.multicastMasterSubscribe = true; 
  ...
  if(multicastSubscribe)
  {
    //Allocate buffers and queue up frames before entering grab loop.
      ...
      if(argc >= 3)
      {
  	  ...
  	  else if (strcmp(argv[2], "BayerBG12Packed") == 0)
  	  {
